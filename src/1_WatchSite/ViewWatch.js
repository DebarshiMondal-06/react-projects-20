...
const ViewWatch = () => {
  ...
  const { refetch } = useContext(createGlobalContext);


  var [addUserCartData] = useMutation(ADD_TO_CART);
  var [speceficCartCount] = useMutation(LOAD_SPECEIFIC_COUNT);
  const { loading, data } = useQuery(LOAD_ONE_WATCH, {
    variables: { id },
    context: {
      headers: {
  ...
  const count_specefic = (user, watch) => {
    speceficCartCount({
      ...
      const { speceficCartCount } = res.data;
      if (speceficCartCount && speceficCartCount.count > 0) setValue(speceficCartCount.count);
      else setValue(0);
    })
  }
  useEffect(() => {
    if (id && cookie.authToken && cookie.userevent) {
      count_specefic(cookie.userevent, id);
    }
  }, [id, cookie]);
  ...
  const update_cart = () => {
    addUserCartData({
      variables: {
        user_id: cookie && cookie.userevent,
        watch_id: id,
        count: value,
      },
      context: {
  ...
  const { brand, name, description, price, category } = data.loadOneWatch;
